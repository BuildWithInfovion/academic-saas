    ...
    <div className="p-8 max-w-5xl">
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          ...
          { label: 'Announcements', desc: 'School-wide notices', path: '/portal/principal/announcements', color: 'bg-gray-700 text-white' },
        ].map((a) => (
          <button
            key={a.label}
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-800 text-sm">Attendance Defaulters</h2>
            <p className="text-xs text-gray-400 mt-0.5">Below 75% this month</p>
          </div>
          <div className="divide-y divide-gray-50">
            {attendanceDefaulters.length === 0 ? (
              <p className="text-sm text-gray-400 px-5 py-4">No defaulters this month.</p>
            ) : attendanceDefaulters.map((d) => (
              <div key={d.id} className="px-5 py-3 flex items-center justify-between">
                <span className="text-sm text-gray-800">{d.firstName} {d.lastName}</span>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm">
          ...
          <div className="divide-y divide-gray-50">
            {feeDefaulters.length === 0 ? (
              <p className="text-sm text-gray-400 px-5 py-4">No fee defaulters found.</p>
            ) : feeDefaulters.map((d) => (
              <div key={d.id} className="px-5 py-3 flex items-center justify-between">
                <span className="text-sm text-gray-800">{d.firstName} {d.lastName}</span>
