...
const inp = 'border border-ds-border-strong p-2 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-ds-brand w-full';
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-ds-text1">Classes</h1>
          <p className="text-sm text-ds-text3 mt-0.5">Manage classes, sections and assign class teachers</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-ds-surface rounded-xl border border-ds-border shadow-sm p-12 text-center">
          <p className="text-ds-text2 font-medium mb-1">No classes yet</p>
          <p className="text-sm text-ds-text3">Click "+ Add Class / Section" to get started.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {groupEntries.map(([parentName, units]) => (
            <div key={parentName} className="bg-ds-surface rounded-xl border border-ds-border shadow-sm overflow-hidden">
              <div className="px-5 py-3 border-b border-ds-border bg-ds-bg2 flex items-center justify-between">
                <span className="text-xs font-semibold text-ds-text2 uppercase tracking-wider">{parentName}</span>
                <span className="text-xs text-ds-text3">{units.length} class{units.length !== 1 ? 'es' : ''}</span>
              </div>
              <div className="divide-y divide-ds-border">
                {units.map((cls) => (
                  <div key={cls.id} className="flex items-center justify-between px-5 py-4 hover:bg-ds-bg2">
                    <div className="flex-1">
                      <p className="font-medium text-ds-text1 text-sm">{cls.displayName || cls.name}</p>
                      {cls.classTeacher ? (
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-medium border border-ds-border-strong rounded-lg hover:bg-ds-bg2 transition-colors"
                      >
                        {cls.classTeacher ? 'Change Teacher' : 'Assign Teacher'}
                      </button>
                      <button
                        onClick={() => deleteUnit(cls)}
                        className="px-3 py-1.5 text-xs font-medium border border-red-200 text-red-500 rounded-lg hover:bg-ds-error-bg transition-colors"
                      >
                        Delete
                      </button>
                    </div>
      ...
      {assigningUnit && (
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-96 bg-ds-surface h-full shadow-xl flex flex-col">
            <div className="px-6 py-5 border-b border-ds-border">
              <h2 className="font-semibold text-ds-text1">Assign Class Teacher</h2>
              <p className="text-xs text-ds-text3 mt-0.5">{assigningUnit.displayName || assigningUnit.name}</p>
            </div>
            <div className="flex-1 px-6 py-5 space-y-4 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-semibold text-ds-text2 mb-2 uppercase tracking-wider">Select Teacher</label>
                {teachers.length === 0 ? (
            ...
            <div className="px-6 py-4 border-t border-ds-border flex gap-3">
              <button onClick={saveClassTeacher} disabled={saving}
                className="btn-brand flex-1 py-2 rounded-lg">
                {saving ? 'Saving...' : 'Save'}
              </button>
              <button onClick={() => setAssigningUnit(null)}
                className="px-4 py-2 border border-ds-border-strong rounded-lg text-sm hover:bg-ds-bg2">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-ds-surface rounded-2xl shadow-2xl w-full max-w-lg p-6">

            {/* Step 1: pick type */}
            {!createType && (
              <>
                <h2 className="font-semibold text-ds-text1 text-lg mb-1">What do you want to add?</h2>
                <p className="text-sm text-ds-text3 mb-5">Choose the type of unit to create</p>
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <button
                    onClick={() => setCreateType('class')}
                    className="border-2 border-ds-border hover:border-black rounded-xl p-5 text-left transition-colors group"
                  >
                    <p className="text-2xl mb-2">🏫</p>
                    <p className="font-semibold text-ds-text1 group-hover:text-black">New Class</p>
                    <p className="text-xs text-ds-text3 mt-1">e.g. Class 1, Class 2, Grade 10, Nursery</p>
                  </button>
                  <button
                  ...
                  >
                    <p className="text-2xl mb-2">🔤</p>
                    <p className="font-semibold text-ds-text1 group-hover:text-black">New Section / Division</p>
                    <p className="text-xs text-ds-text3 mt-1">e.g. Class 1 - A, Class 1 - B, Rose, Lotus</p>
                  </button>
                </div>
                <button onClick={resetCreate} className="text-sm text-ds-text3 hover:text-ds-text2">Cancel</button>
              </>
            )}
            ...
            {createType === 'class' && (
              <>
                <button onClick={() => setCreateType(null)} className="text-xs text-ds-text3 hover:text-ds-text2 mb-4">← Back</button>
                <h2 className="font-semibold text-ds-text1 text-lg mb-1">Add New Class</h2>
                <p className="text-sm text-ds-text3 mb-5">Enter the class name as students and teachers will see it</p>
                <div className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-ds-text2 mb-1">Class Name *</label>
                    <input
                      className={inp}
                    ...
                    />
                    {newClassName.trim() && (
                      <p className="text-xs text-ds-text3 mt-1">Will appear as: <span className="font-medium text-ds-text2">{newClassName.trim()}</span></p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-3 pt-1">
                    ...
                    <button onClick={resetCreate} className="px-4 py-2 border border-ds-border-strong rounded-lg text-sm hover:bg-ds-bg2">Cancel</button>
                  </div>
                </div>
              </>
            )}
            ...
            {createType === 'section' && (
              <>
                <button onClick={() => setCreateType(null)} className="text-xs text-ds-text3 hover:text-ds-text2 mb-4">← Back</button>
                <h2 className="font-semibold text-ds-text1 text-lg mb-1">Add Section / Division</h2>
                <p className="text-sm text-ds-text3 mb-5">Choose which class this section belongs to, then name it</p>

                {topLevelUnits.length === 0 ? (
                  <div className="bg-ds-warning-bg border border-ds-warning-border rounded-xl p-4 mb-4">
                    <p className="text-sm text-ds-warning-text font-medium">No classes exist yet</p>
                    <p className="text-xs text-ds-warning-text mt-1">Create a class first (e.g. "Class 1"), then come back to add sections.</p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-ds-text2 mb-1">Class *</label>
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-ds-text2 mb-1">Section Name *</label>
                      <input
                        className={inp}
                      ...
                      {newSectionParent && newSectionName.trim() && (
                        <p className="text-xs text-ds-text3 mt-1">
                          Will appear as: <span className="font-medium text-ds-text2">
                ...
                {topLevelUnits.length === 0 && (
                  <button onClick={resetCreate} className="mt-2 px-4 py-2 border border-ds-border-strong rounded-lg text-sm hover:bg-ds-bg2">Cancel</button>
                )}
              </>
            )}
          </div>
